...
// firmly 2026-09-14) that piano-only as the SOLE instrument family reads as too uniformly
// calm across every reel. This is now a rotation of acoustic-only instrument families, not
// one locked prompt -- each render picks one variant, so consecutive reels don't all sound
// the same, while every variant still stays inside the same organic/never-corporate floor.
const MUSIC_STYLE_VARIANTS = [
  "warm, organic, acoustic instrumental only -- solo piano, intimate and slightly " +
    ...
    "tech-demo sounding",
  "warm, organic, acoustic instrumental only -- soft felt piano, close and hushed, warm " +
    "room reverb, restrained and human, never corporate or tech-demo sounding",
  ...
  "warm, organic, acoustic instrumental only -- felt piano with soft pizzicato strings " +
    "underneath, a little more movement and brightness than pure piano, still intimate, " +
    "never corporate or tech-demo sounding",
  "warm, organic, acoustic instrumental only -- acoustic guitar with a light hand-percussion " +
    "pulse (soft cajon or brushed rhythm, no drum kit, no electronic drums), more rhythmic " +
    "and upbeat energy than a ballad while staying warm and human, never corporate or " +
    "tech-demo sounding",
];

function pickMusicStyleVariant() {
  return MUSIC_STYLE_VARIANTS[Math.floor(Math.random() * MUSIC_STYLE_VARIANTS.length)];
}
