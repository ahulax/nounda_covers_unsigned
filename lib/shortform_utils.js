...
// Gemini attaches punctuation to the word itself ("stall,", "frame."), so a word ending
// in terminal punctuation marks a sentence end. Quotes/brackets may trail it.
const SENTENCE_END = /[.!?]["'”’)\]]?$/;

/**
 * Snap a requested [start, end] onto real sentence boundaries taken from the word-level
 * transcript. The segment-picking LLM chooses times from the analysis JSON and lands
 * mid-sentence often enough to matter: a clip joined to the next one cuts a sentence in
 * half, and the last sentence of the video gets truncated while its caption still renders
 * in full (captions are word-derived, audio is not). Snapping both edges here fixes both,
 * and keeps audio and captions derived from the identical window.
 *
 * Returns the original bounds unchanged when the transcript can't support a better
 * choice, so this can never make a clip worse than the requested one.
 */
function snapToSentence(words, start, end, maxSeconds = 60, tailPad = 0.55) {
  const list = Array.isArray(words) ? words.filter((w) => w && typeof w.start === "number") : [];
  if (list.length === 0) return { start, end };

  const startsSentence = list.map((_, i) => i === 0 || SENTENCE_END.test(list[i - 1].word || ""));

  // Begin at the opening of the sentence the requested start falls inside.
  let snappedStart = null;
  for (let i = 0; i < list.length; i++) {
    if (startsSentence[i] && list[i].start <= start + 0.2) snappedStart = list[i].start;
  }
  if (snappedStart === null) snappedStart = Math.min(start, list[0].start);

  // Run to the close of the sentence still in progress at the requested end, rather than
  // stopping mid-clause. tailPad keeps the final word from being clipped by the encoder.
  let snappedEnd = null;
  for (let i = 0; i < list.length; i++) {
    if (SENTENCE_END.test(list[i].word || "") && list[i].end >= end - 0.2) {
      snappedEnd = list[i].end + tailPad;
      break;
    }
  }
  if (snappedEnd === null || snappedEnd - snappedStart > maxSeconds) {
    // Extending would overrun the length cap (or there is no later sentence end), so fall
    // back to the last sentence that completes inside the requested window.
    let fallback = null;
    for (const w of list) {
      if (SENTENCE_END.test(w.word || "") && w.end <= end + 0.2) fallback = w.end + tailPad;
    }
    snappedEnd = fallback !== null ? fallback : end;
  }
...
// explicitly rejected anything synth/electronic-sounding as "corporate 2015" stock-video
// music -- that boundary never relaxes. But Daniil flagged (2026-09-12, repeated more
// firmly 2026-09-14 and 2026-09-16) that piano-only reads as too uniformly calm and boring
// across every reel. A first attempt (2026-09-14) rotated across 5 variants but picked
// randomly per render -- two real problems Daniil's follow-up exposed: (1) 3 of the 5
// variants still centered on piano, so "random" could easily land on piano-ish results
// repeatedly by chance, and (2) independent random draws give no guarantee against
// back-to-back repeats, so "did it actually change" wasn't verifiable, only probabilistic.
// Fixed now: only 2 of 5 variants involve piano at all, and selection is DETERMINISTIC by
// calendar day (not random), so it's directly checkable -- today's style and tomorrow's
// style are never the same, and which variant is "today" can be stated and verified.
const MUSIC_STYLE_VARIANTS = [
  "warm, organic, acoustic instrumental only -- solo piano, intimate and slightly " +
    "melancholic, unhurried, warm room reverb, restrained and human, never corporate or " +
    "tech-demo sounding",
  "warm, organic, acoustic instrumental only -- fingerstyle acoustic guitar, gentle " +
    "forward momentum, light natural room tone, never corporate or tech-demo sounding",
  "warm, organic, acoustic instrumental only -- warm string ensemble (violin and cello, " +
    "pizzicato-forward, no piano at all), light and airy, never corporate or tech-demo " +
    "sounding",
  "warm, organic, acoustic instrumental only -- acoustic guitar with a light hand-percussion " +
    "pulse (soft cajon or brushed rhythm, no drum kit, no electronic drums, no piano), more " +
    "rhythmic and upbeat energy than a ballad while staying warm and human, never corporate " +
    "or tech-demo sounding",
  "warm, organic, acoustic instrumental only -- felt piano with soft pizzicato strings " +
    "underneath, more movement and brightness than solo piano, still intimate, never " +
    "corporate or tech-demo sounding",
];

function pickMusicStyleVariant() {
  const dayIndex = Math.floor(Date.now() / 86400000); // days since Unix epoch, UTC
  return MUSIC_STYLE_VARIANTS[dayIndex % MUSIC_STYLE_VARIANTS.length];
}
...
async function generateMusicBed(durationSeconds, workDir, moodNote) {
  const baseStyle = pickMusicStyleVariant();
  const prompt = moodNote ? `${baseStyle}. ${moodNote}` : baseStyle;
...
module.exports = {
  ...
  parseSegmentAnalysis, wordsInWindow, chunkWords, snapToSentence, fetchAirtableRecord,
